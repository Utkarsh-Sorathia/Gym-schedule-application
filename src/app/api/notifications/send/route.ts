...
export async function POST(request: Request) {
    try {
        ...
        let workoutFocus = "Your scheduled workout";
        ...
        try {
            ...
            if (scheduleData.success && scheduleData.data && scheduleData.data.length > 0) {
                ...
                // Extract the workout focus from the day (e.g., "Monday (Chest & Triceps)" -> "Chest & Triceps")
                const focusMatch = schedule.day.match(/\(([^)]+)\)/);
                if (focusMatch) {
                    workoutFocus = focusMatch[1];
                }
            }
        } catch (error) {
